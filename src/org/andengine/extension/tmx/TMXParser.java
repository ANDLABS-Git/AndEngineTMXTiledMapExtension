...
public class TMXParser extends DefaultHandler implements TMXConstants {
    ...
    public TMXParser(final AssetManager pAssetManager, final TextureManager pTextureManager,
    ...
    public void startElement(final String pUri, final String pLocalName, final String pQualifiedName,
        ...
        if (pLocalName.equals(TMXConstants.TAG_MAP)) {
            ...
            if (tsxTileSetSource == null) {
                ...
                try {
                    final int firstGlobalTileID = SAXUtils.getIntAttribute(pAttributes,
    ...
    public void endElement(final String pUri, final String pLocalName, final String pQualifiedName) throws SAXException {
        if (pLocalName.equals(TMXConstants.TAG_MAP)) {
            ...
            if (binarySaved) {
                final ArrayList<TMXLayer> tmxLayers = this.mTMXTiledMap.getTMXLayers();
                try {
                    tmxLayers.get(tmxLayers.size() - 1).initializeTMXTilesFromDataString(this.mStringBuilder.toString().trim(),
                            this.mDataEncoding, this.mDataCompression, this.mTMXTilePropertyListener);
